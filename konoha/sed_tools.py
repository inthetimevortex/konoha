...
def color_corr(lbd, flux, flag):
    ...
    if (flag.lower() == "akari") or (flag.lower() == "akari/irc"):
        if len(lbd) == 1:
            ...
            lbdi = np.array([9.0, 18.0])
            flux_corr = (
                flux / K[np.where(np.abs(lbd - lbdi) == np.min(np.abs(lbd - lbdi)))]
            )
            return flux_corr
        else:
            band = [dir0 + "bandpasses/irc-s9w.dat", dir0 + "bandpasses/irc-l18w.dat"]
    elif flag.lower() == "iras":
        if len(lbd) == 1:
            print("Just one data point: assuming slope=3")
            K = np.array([1.25, 1.23, 1.15, 1.04])
            lbdi = np.array([12.0, 25.0, 60.0, 100.0])
            flux_corr = (
                flux / K[np.where(np.abs(lbd - lbdi) == np.min(np.abs(lbd - lbdi)))]
            )
            return flux_corr
            ...
            for i in range(len(lbd_arr)):
                if (np.abs(lbd - np.float(lbd_arr[i])) < delt).any():
                    band.append(dir0 + "bandpasses/IRAS_IRAS." + lbd_arr[i] + "mu.dat")
        ...
        if len(lbd) == 1:
            ...
            for i in range(4):
                if (np.abs(lbd - lbdi[i]) < delt).any():
                    band.append(
                        dir0 + "bandpasses/RSR-W" + "{:1d}".format(i + 1) + ".txt"
                    )
    else:
    ...
    while res > delt:
        for iband in range(nband):
            ...
            lbdi, R = tab[:, 0], tab[:, 1]
            logG = poly_interp(
                np.log(lbd), np.log((flux / K) / (flux[iband] / K[iband])), np.log(lbdi)
            )
            G = np.exp(logG)
            if flag.lower() == "wise":
                K1[iband] = integral(lbdi, R * G * lbdi) / integral(lbdi, R * lbdi)
            else:
                K1[iband] = integral(lbdi, R * G) / integral(
                    lbdi, R * (lbd[iband] / lbdi)
                )
...
def vosa2catvalues(lbd, flux, flag):
    ...
    delt = 1.0  # wavelength tolerance range
    ...
    if (flag.lower() == "akari") or (flag.lower() == "akari/irc"):
        ...
        if i09.any():
            lcat[i09] = 9.0
            fcat[i09] = flux[i09] * (lbd[i09] / 9.0) ** 2
        if i18.any():
            lcat[i18] = 18.0
            fcat[i18] = flux[i18] * (lbd[i18] / 18.0) ** 2
        ...
        if i65.any():
            lcat[i65] = 65.0
            fcat[i65] = flux[i65] * (lbd[i65] / 65.0) ** 2
        if i90.any():
            lcat[i90] = 90.0
            fcat[i90] = flux[i90] * (lbd[i90] / 90.0) ** 2
        if i140.any():
            lcat[i140] = 140.0
            fcat[i140] = flux[i140] * (lbd[i140] / 140.0) ** 2
        if i160.any():
            lcat[i160] = 160.0
            fcat[i160] = flux[i160] * (lbd[i160] / 160.0) ** 2
    elif flag.lower() == "iras":
        ...
        if i12.any():
            lcat[i12] = 12.0
            fcat[i12] = flux[i12] * (lbd[i12] / 12.0) ** 2
        if i25.any():
            lcat[i25] = 25.0
            fcat[i25] = flux[i25] * (lbd[i25] / 25.0) ** 2
        if i60.any():
            lcat[i60] = 60.0
            fcat[i60] = flux[i60] * (lbd[i60] / 60.0) ** 2
        if i100.any():
            lcat[i100] = 100.0
            fcat[i100] = flux[i100] * (lbd[i100] / 100.0) ** 2
    elif flag.lower() == "wise":
        lcat = lbd
        fcat = flux
    else:
