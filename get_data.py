...
from konoha.utils import bin_data, integral, poly_interp, geneva_interp_fast
from konoha.sed_tools import vosa2catvalues, color_corr, jy2cgs
from konoha.radial_velocity import * 
...
def get_iue():
    ...
    nbins = 200
    xbin, ybin, dybin = bin_data(lbd_iue, flux_iue, nbins,
                                 exclude_empty=True)
...
def get_lines(line):
    ...
    if flag in USE:
        ...
        for n in range(len(lines)):
            ...
            lbd = fits_header['CRVAL1'] + fits_header['CDELT1'] * np.arange(len(fits_data))
...
def get_sed():
    ...
    # read data
    data = np.loadtxt(fname, dtype={'names':('filter', 'lbd', 'flux', 'dflux'), \
                      'formats':('S20', np.float, np.float, np.float)}, skiprows=10)
...
def plot_lines(line, togle):
    ...
    for k in range(len(selects)):
        ...
        for i_MJD in range(selects[k][0],selects[k][1]):
            ...
            ax1.plot(vel , flux_all[i_MJD] + 0.01*i_MJD, color=cor[int(flags[i_MJD])], lw=0.8)
            
            ax1.set_xlim(-800, 800)
    
        
        patches = [mpatches.Patch(color=cor[i], label=flag_names[i]) for i in range(len(flag_names))]
        ax1.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0., handles=patches)
        ax1.set_xlabel('$\mathrm{vel\,[km\,s^{-1}]}$')
        ax1.set_ylabel('$\mathrm{Relative\, flux}$')
        plt.savefig('{0}_{1}_profiles.png'.format(line, k), dpi=200, bbox_inches='tight')
        #plt.show()
...
def gen_plot_lines(line, MJD_all,vel_all, flux_all, flag_all):
    ...
    for i_MJD in range(n_MJD):
        ...
        ax1.plot(vel , flux_all[i_MJD] + 0.2*i_MJD, color=cor[int(flags[i_MJD])], lw=0.8)
